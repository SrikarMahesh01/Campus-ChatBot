...
`;
  ...
  public async generateResponse(query: string): Promise<ChatResponse> {
    try {
      ...
      // First, check for a predefined response for an exact match.
      const predefinedResponse = this.findResponse(query);
      if (predefinedResponse) {
        console.log('📋 Found predefined response');
  ...
  private findResponse(query: string): ChatResponse | undefined {
